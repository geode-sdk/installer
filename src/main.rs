...
// Style Utils
use iced::{alignment::Horizontal, Alignment, Color, Length, Padding, Theme};
...
impl Sandbox for MainWindow {
	...
	fn view(&self) -> Element<Self::Message> {
		...
		let col = Column::new()
			...
			.push(
				Svg::new(svg::Handle::from_memory(Cow::from(
		...
		match self.page {
			...
			MessageType::Install | MessageType::Uninstall => {
				...
				let return_btn = Button::new(Text::new("Back").size(20))
					.style(MyBtnStyle::theme())
					.padding(Padding::from([5, 10]))
					.on_press(MessageType::Main);

				col.push(Text::new(text).style(color))
					.spacing(20)
					.push(
						Container::new(return_btn)
							.height(Length::Fill)
							.center_y()
							.center_x(),
					)
